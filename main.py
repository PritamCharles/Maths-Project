...
class Interface:
    ...
    def mw_butt_action(self):
        ...
        if self.main_win.combo1_value == "Puissance itérée":
            ...
            nbiter, eigvec, heigval, fepsilon = self.iter_pow.iterated_power_method()
            self.iterpow_win.nbiter = nbiter
            self.iterpow_win.heigvec = eigvec
            ...
            if self.main_win.combo_invm.get() == "Cholesky":
                self.invpow_win.combo_invm_value = self.main_win.combo_invm_value
            elif self.main_win.combo_invm.get() == "LU":
                self.main_win.combo_invm_value = self.main_win.combo_invm.get()
                self.invpow_win.combo_invm_value = self.main_win.combo_invm_value
            elif self.main_win.combo_invm.get() == "QR":
                self.main_win.combo_invm_value = self.main_win.combo_invm.get()
                self.invpow_win.combo_invm_value = self.main_win.combo_invm_value
            ...
            self.invpow_win.eigvec = eigvec
            self.invpow_win.heigval = heigval
